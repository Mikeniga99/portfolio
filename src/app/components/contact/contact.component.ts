...
@Component({
                ...
                <div class="item-perspective">
                  <div class="reveal contact-item" data-reveal="tilt" appInView [style.--d]="(i * 0.1) + 's'"
                    ...
                    <div>
                      ...
                      @if (item.href) {
                        <a [href]="item.href" class="value link">{{ item.value }}</a>
                      } @else {
  ...
  styles: [`
    ...
    .grid-bg { position: absolute; inset: 0; pointer-events: none; background-image: linear-gradient(rgba(0,200,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,200,255,0.03) 1px, transparent 1px); background-size: 60px 60px; }
    .radial-bg { position: absolute; inset: 0; pointer-events: none; background: radial-gradient(ellipse 60% 60% at 50% 50%, rgba(123,47,255,0.04) 0%, transparent 70%); }
    ...
    .contact-item {
      ...
      background: var(--card); transition: border-color 0.3s ease, background 0.3s ease;
    }
    .contact-item:hover { border-color: var(--primary); background: rgba(0,200,255,0.06); }
    ...
    .chip { padding: 8px 16px; border: 1px solid var(--border); font-size: 14px; color: var(--muted-foreground); font-family: var(--font-mono); }

    .cta-perspective { perspective: 1000px; }
    .cta-card {
      border: 1px solid rgba(0,200,255,0.2); padding: 40px; position: relative; overflow: hidden;
      height: 100%; min-height: 256px; display: flex; flex-direction: column; justify-content: space-between;
      background: linear-gradient(135deg, rgba(0,200,255,0.04) 0%, rgba(123,47,255,0.04) 100%);
    }
    .cta-glow { position: absolute; inset: 0; pointer-events: none; background: radial-gradient(circle at 70% 30%, rgba(0,200,255,0.06) 0%, transparent 60%); }
...
export class ContactComponent {
  ...
  items: ContactItem[] = [
    { label: 'Email', value: 'mikelsossou1@gmail.com', href: 'mailto:mikelsossou1@gmail.com', icon: this.sanitizer.bypassSecurityTrustHtml(SVG_ICONS.mail), accent: 'var(--primary)' },
    { label: 'Téléphone', value: '+229 01 999 317 15', href: 'tel:+22901999317 15', icon: this.sanitizer.bypassSecurityTrustHtml(SVG_ICONS.phone), accent: 'var(--secondary)' },
    { label: 'Localisation', value: 'Cotonou, Bénin', href: null, icon: this.sanitizer.bypassSecurityTrustHtml(SVG_ICONS.mapPin), accent: 'var(--accent)' },
  ];
